...
export class BudgetCommandService {
  ...
  /*  async create(createBudgetDto: CreateBudgetDto, userId: string): Promise<BudgetResponseDto> {
     ...
     const budgetData = {
       ...
       categoryBudgets:
         createBudgetDto.categoryBudgets?.map(catBudget => ({
           ...
           spentAmount: 0,
           trackTransactions: catBudget.trackTransactions ?? true,
           alerts:
   ...
   /* async recalculateBudgetSpending(budgetId: string): Promise<void> {
     ...
     const transactions = await this.transactionsService.findByProfile(budget.accountId.toString(), {
       startDate: budget.startDate,
       endDate: budget.endDate,
       limit: 10000 // Get all transactions for this period
     });
 
     // Calculate total spending
     const totalSpent = transactions.data.reduce((sum, transaction) => sum + transaction.amount, 0);
 
     // Calculate category spending
     const categorySpending = new Map<string, number>();
     transactions.data.forEach(transaction => {
       const categoryId = transaction.categoryId.toString();
       categorySpending.set(categoryId, (categorySpending.get(categoryId) || 0) + transaction.amount);
     });
