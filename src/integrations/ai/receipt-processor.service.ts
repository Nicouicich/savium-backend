...
export class ReceiptProcessorService {
  ...
  /**
   * Process an image to determine if it's a receipt and create transaction if so
   */
       ...
       if (extractedData.amount && extractedData.amount > 0) {
         try {
           ...
           await this.updateFileMetadata(
             uploadResult.fileId,
             {
               tags: ['whatsapp', 'receipt', 'processed', 'transaction_created'],
               purpose: FilePurpose.RECEIPT,
               description: `Receipt processed - Transaction #${transactionCreated.transactionId}`
             },
             userId,
             profileId
           );
         } catch (transactionError) {
           this.logger.error('Failed to create transaction from receipt', {
             traceId,
  ...
  /*   private async createTransactionFromReceipt(
      ...
      profileId: string,
      traceId?: string
    ): Promise<{ transactionId: string; amount: number; description: string }> {
      try {
        this.logger.log('Creating transaction from receipt data', {
          traceId,
          amount: extractedData.amount,
          vendor: extractedData.vendor,
          fileId
        ...
        const transactionData = {
          ...
          attachedFiles: [fileId], // Use attachedFiles as expected by TransactionsService
   ...
   */
  /* private async getCategoryByName(categoryName: string, profileId: string): Promise<string> {
    try {
